...
// step = 1 → (kullanılmıyor ama ileride gerekirse)
...
function handleUnlock() {
  ...
  if (!codeMap[rawCode]) {
    ...
    // Kaç hak var? (quiz sonrası +1 dahil)
    const maxTotal = quizExtraGiven ? maxAttempts + 1 : maxAttempts;
    const kalan = maxTotal - attempts;

    // Haklar tamamen bittiyse
    if (attempts >= maxTotal) {
      if (!quizExtraGiven) {
        // İlk kez 3 hataya ulaştı → Quiz aç
        showWrongCode("3 kez yanlış girdin. Mini quiz birazdan açılacak.");
        openQuiz();
        return;
      } else {
        // Quiz sonrası da yanlış yaptı → kalıcı kilit
        showWrongCode(
